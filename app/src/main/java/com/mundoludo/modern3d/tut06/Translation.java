...
public class Translation extends Framework {
    ...
    private OffsetFunc StationaryOffset =
        (Float fElapsedTime) -> new Vec3(0f,0f, -20.0f);

    private OffsetFunc OvalOffset = (Float fElapsedTime) -> {
	float fLoopDuration = 3.0f;
	float fScale = (float) Math.PI * 2.0f / fLoopDuration;

	float fCurrTimeThroughLoop = fElapsedTime % fLoopDuration;

	return new Vec3(
            (float) Math.cos(fCurrTimeThroughLoop * fScale) * 4,
            (float) Math.sin(fCurrTimeThroughLoop * fScale) * 6,
            -20.0f
        );
    };

    private OffsetFunc BottomCircleOffset = (Float fElapsedTime) -> {
        ...
        return new Vec3(
            Math.cos(fCurrTimeThroughLoop * fScale) * 5,
            -3.5f,
            Math.sin(fCurrTimeThroughLoop * fScale) * 5 - 20.0f
        );
    };
    ...
    private class Instance {
        private OffsetFunc calcOffset;
        private Vec3 vec = new Vec3();

        Instance(OffsetFunc OffsetFunc) {
            this.calcOffset = OffsetFunc;
        }

        Mat4 ConstructMatrix(float fElapsedTime) {

            Mat4 theMat = new Mat4(1.0f);
            theMat.set(3, new Vec4(calcOffset.execute(fElapsedTime), 1.0f));
